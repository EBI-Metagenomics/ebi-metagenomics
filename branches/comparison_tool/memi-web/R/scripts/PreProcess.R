...
# Can be used for GOs (.csv files) and IPR (.tsv files)
...
# FROM OLD VERSION: Do we want the names of samples? If yes, keep them. If not, just name samples 'Sample 1' to 'Sample i'
#   if(keepNames == TRUE) {
#     sampleNames <- regmatches(fileList, regexpr('[^/]+\\.', fileList))
#     sampleNames <- paste('S',sprintf("%02d",1:size),' (',gsub('_.+','\\1',sampleNames),')',sep='')
#   }
#   else {
#     sampleNames <- paste('Sample',1:size)
#   }
  ...
  # If used data is 'GOslim', re-order rows following the order of GOslimTable (defined in launch script)
  if (usedData=='GOslim') {
     abTable = abTable[GOslimTable$GO,,drop=FALSE]
  }  
...
NewGOTable <- function(goFile) {
  ...
  # Delete useless columns from it
  newTable$V2 <- NULL
  newTable$V3 <- NULL
...
NewIPRTable <- function(i5tsvFile) {
  # Extracts the occurrence of each IPR in a sample from an 'InterPro matches summary' file (TSV) and store
  # this data as a table.  
  # 
  # Args: 
  #   i5tsvFile: an InterPro matches file (TSV).
  # 
  # Returns: 
  #   A table containing the abundance of IPR for a single sample.
  
  newTable <- fread(i5tsvFile, header = FALSE, sep = ",", stringsAsFactors = FALSE)
  # Delete useless column from it
  newTable$V2 <- NULL
  setnames(newTable, colnames(newTable), c("IPR", "count"))  
  newTable$count <- as.numeric(newTable$count)
  # Return the table
  return(newTable)
}
