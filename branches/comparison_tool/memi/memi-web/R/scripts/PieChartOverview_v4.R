...
message(paste(Sys.time(),'[R - Message] Launched R script PieChartOverview_v3.R'))
...
# To be continued...
    ...
    tmpFileName = 'R/tmpGraph/tempOverview.htm'
    a$save(tmpFileName)
    tmpFileContent = paste(readLines(tmpFileName),collapse='\n')
    chartCode <- regmatches(tmpFileContent, regexpr('<div id=\'chart.+</script>',tmpFileContent))
    # Delete !
