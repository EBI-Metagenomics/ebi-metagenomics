...
public class EmgLogFileInfoDAOImpl implements EmgLogFileInfoDAO {
    ...
    public List<String> getFileIdsBySampleId(String sampleId) {
        ...
        try {
            List<Map<String, Object>> rows = this.jdbcTemplate.queryForList("select file_id from " + EmgLogFileInfoDAO.TABLE_NAME + " where sample_id=?", new String[]{sampleId});
            for (Map row : rows) {
    ...
    public List<String> getFileNamesBySampleId(String sampleId) {
        ...
        List<String> result = new ArrayList<String>();
        try {
            List<Map<String, Object>> rows = this.jdbcTemplate.queryForList("select file_name from " + EmgLogFileInfoDAO.TABLE_NAME + " where sample_id=?", new String[]{sampleId});
            for (Map row : rows) {
    ...
    public List<EmgFile> getFilesBySampleId(String sampleId) {
        ...
        try {
            List<Map<String, Object>> rows = this.jdbcTemplate.queryForList("select file_id, file_name from " + EmgLogFileInfoDAO.TABLE_NAME + " where sample_id=?", new String[]{sampleId});
            for (Map row : rows) {
    ...
    public List<String> getSraIDs(String sampleId) {
        ...
        List<String> result = new ArrayList<String>();
        try {
            List<Map<String, Object>> rows = this.jdbcTemplate.queryForList("select sra_run_ids from " + EmgLogFileInfoDAO.TABLE_NAME + " where sample_id=?", new String[]{sampleId});
            for (Map row : rows) {
