...
<div id="tax-col">
    ...
    <div  class="chart_container" >
        <div class="grid_24"><div id="tax_lsu_chart_col" style="height: 360px; width:70%; margin:0 auto;"></div></div>
        <div class="grid_24"> <table id="tax_table_lsu_col" class="table-glight"></table></div>
    </div>
...
<script type="text/javascript">
    ...
    $(document).ready(function() {
        ...
        var t = $('#tax_table_lsu_col').DataTable( {
            order: [[ 3, "desc" ]],
        ...
        $("#tax_table_lsu_col tbody").on('click', 'tr', function(){
...
//            var index = $(this).index();
            var chart = $('#tax_lsu_chart_col').highcharts();
        ...
        $("#tax_table_lsu_col tbody").on('mouseenter', 'tr', function(){
            var legInd = (this).className.split(' ')[0];
            var chart = $('#tax_lsu_chart_col').highcharts();
        ...
        $("#tax_table_lsu_col tbody").on('mouseleave', 'tr', function(){
            var legInd = (this).className.split(' ')[0];
            var chart = $('#tax_lsu_chart_col').highcharts();
            var point = chart.series[legInd].points[0];
            point.setState('');
        ...
        $("#tax_table_lsu_col_filter input").addClass("filter_sp");
        ...
        $('.filter_sp').keyup(function () {
            $("#tax_table_lsu_col tr td:nth-child(n+2)").highlight($(this).val());
            $('#tax_table_lsu_col tr td:nth-child(n+2)').unhighlight();// highlight more than just first character entered in the text box and reiterate the span to highlight
            $('#tax_table_lsu_col tr td:nth-child(n+2)').highlight($(this).val());
        });
        // remove highlight when click on X (clear button)
        $('input[type=search]').on('search', function () {
            $('#tax_table_lsu_col tr td').unhighlight();
        });
    } );
        ...
        $(document).ready(function () {
            ...
            var data = [
                ...
                { name: '${taxonomyData.phylum}',
                  data: [<fmt:formatNumber type="number" maxFractionDigits="3" value="${taxonomyData.numberOfHits *100/ model.taxonomyAnalysisResultLSU.sliceVisibilityThresholdDenominator}" />],
            ...
            $('#tax_lsu_chart_col').highcharts({
                ...
                tooltip: {
                    ...
                    <c:when test="${model.run.releaseVersion == '1.0'}">
                    formatter:function(){
                        return '<span style="color:'+this.series.color+'">&#9632;</span> '+this.series.name+':<br/><strong>'+Highcharts.numberFormat((this.y/100*${model.taxonomyAnalysisResultLSU.sliceVisibilityThresholdDenominator}),0,'.')+'</strong> OTUs ('+Highcharts.numberFormat((this.percentage),2,'.')+'%)';
                    },
                    </c:when>
                    <c:otherwise>
                    formatter:function(){
                        return '<span style="color:'+this.series.color+'">&#9632;</span> '+this.series.name+':<br/><strong>'+Highcharts.numberFormat((this.y/100*${model.taxonomyAnalysisResultLSU.sliceVisibilityThresholdDenominator}),0,'.')+'</strong> reads ('+Highcharts.numberFormat((this.percentage),2,'.')+'%)';
                    },
                    </c:otherwise>
                    </c:choose>
