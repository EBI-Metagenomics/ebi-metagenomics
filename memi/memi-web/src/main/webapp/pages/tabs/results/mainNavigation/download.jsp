    ...
    <c:choose>
        <c:when test="${not empty model.sample.analysisCompleted}">
            <div class="box-export">
                <p>Here you may download the full set of analysis results files. For the original raw sequence reads,
                    follow the link to ENA. Multi-part files should be concatenated after unzipping.</p>
                <section id="download_sq_analysis">
                    ...
                    <table class="pipeline_table">
                        ...
                        <c:forEach var="downloadLink"
                                   items="${model.downloadSection.sequencesDownloadSection.externalDownloadLinks}"
                                   varStatus="loop">
                            <c:choose>
                                <c:when test="${downloadLink.externalLink}"><%-- external link --%>
                                    <tr class="row-cb">
                                        ...
                                        <td class="col_xs xs_hide"><span class="dna_rna icon icon-conceptual icon-c1"
                                                                         data-icon="d"></span></td>
                                        <td class="col_sm  xs_hide"></td>
                                        <td class="col_sm"></td>
                                        <td><a href="${downloadLink.linkURL}" title="${downloadLink.linkTitle}"><span
                                                class="link icon icon-generic" data-icon="L"></a> <span
                                                class="filesize">
                    (ENA website)</span></td>
                                    </tr>
                                </c:when>
                                ...
                                <span class="show_tooltip dna_rna icon icon-conceptual icon-c1" data-icon="d"
                                    ...
                                    <td><a href="<c:url value="${baseURL}/${downloadLink.linkURL}"/>"
                                           title="${downloadLink.linkTitle}"><span
                        ...
                        <c:forEach var="downloadLinkList"
                            ...
                            <c:choose>
                                ...
                                <span class="show_tooltip dna_rna icon icon-conceptual icon-c1" data-icon="d"
                                    ...
                                    <td class="col_sm"><c:out value="${linkPrefix}"/>
                                        FASTA, ${downloadLinkList[0].numberOfChunks}</td>
                            ...
                            <%-- START unchunked seq files --%>
                        <c:forEach var="downloadLink"
                                   items="${model.downloadSection.sequencesDownloadSection.unchunkedDownloadLinks}"
                                   varStatus="loop">
                            <c:choose>
                                <c:when test="${!downloadLink.externalLink}"><%-- CASE 3 - multiple "single link/no partition" NON ZIP - non ENA  links--%>
                        ...
                        </c:forEach><%--END unchunked seq files --%>
                    ...
                    <c:if test="${(not empty model.downloadSection.functionalDownloadSection.interproscanDownloadLinks || not empty model.downloadSection.functionalDownloadSection.otherDownloadLinks)}">
                        ...
                        <table class="pipeline_table">
                            ...
                            <tr>
                                <th>Name</th>
                                <th class="xs_hide">Data type</th>
                                <th class="xs_hide">Compression <span class="icon icon-generic show_tooltip"
                                                                      data-icon="i"
                                                                      title="File compressed using GZIP -GZ, or GNU Zipped Archive file is a compression utility. GZ was adopted by the GNU Project, and is relatively popular on the Internet. GZIP produces files with a GZ extension, which can be decompressed by GZIP/GNUZIP program."></span>
                                </th>
                                <th>Format</th>
                ...
                <section id="download_tax_analysis">
                    <c:if test="${not empty model.downloadSection.taxonomyDownloadSection.downloadLinks}">
                        ...
                        <table class="pipeline_table">
                            ...
                            <c:forEach var="downloadLink"
                                       items="${model.downloadSection.taxonomyDownloadSection.downloadLinks}"
                                       varStatus="loop">
                                ...
                                <span class="show_tooltip dna_rna icon icon-conceptual icon-c1" data-icon="d"
                                    ...
                                    <td class="col_sm">
                                        ...
                                        <c:choose>
                                            ...
                                            <c:when test="${fn:startsWith(downloadLink.linkTitle, 'Click to download the converted OTU table in HDF5 biom format')}">
