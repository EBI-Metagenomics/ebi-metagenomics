    ...
    <c:choose>
        <c:when test="${not empty model.sample.analysisCompleted}">
            <div class="box-export">
                    ...
                    <table class="pipeline_table">
                         <thead>
                             <tr>
                                 <th>Name</th>
                                 <th>Data type</th>
                                 <th>Compression <span class="icon icon-generic show_tooltip"data-icon="i" title="File compressed using GZIP -GZ, or GNU Zipped Archive file is a compression utility. GZ was adopted by the GNU Project, and is relatively popular on the Internet. GZIP produces files with a GZ extension, which can be decompressed by GZIP/GNUZIP program."></span></th>
                                 <th>Format</th>
                                  <th></th>
                             </tr>
                         </thead>
                         <tbody>
                <%--only for ENA --%>
                <c:forEach var="downloadLink" items="${model.downloadSection.sequencesDownloadSection.otherDownloadLinks}" varStatus="loop">
                 <c:choose>
                     <c:when test="${downloadLink.externalLink}"><%-- external link --%>
                                <tr class="row-cb">
                                 ...
                                 <td class="col_sm"></td>
                                 <td class="col_sm"></td>
                                 <td> <a href="${downloadLink.linkURL}" title="${downloadLink.linkTitle}"><span class="link icon icon-generic" data-icon="L"></a> <span class="filesize">
                    (ENA website)</span></td>
                    </tr>
                     </c:when>
                      ...
                      <td class="col_xs">
                          ...
                          <c:choose>
                              <c:when test="${downloadLink.linkText == 'Predicted CDS' || downloadLink.linkText ==  'Predicted CDS without annotation'}">
                                  <span class="show_tooltip protein icon icon-conceptual icon-c3" data-icon="P" title="Protein (AA) data type"></span>
                              </c:when>
                           <c:otherwise>
                               <span class="show_tooltip dna_rna icon icon-conceptual icon-c1" data-icon="d" title="DNA or RNA data type"></span>
                           </c:otherwise>
                          </c:choose>
                      </td>
                      <td class="col_sm">-</td>
                      <td class="col_sm">FASTA</td>
                      <td><a href="<c:url value="${baseURL}/${downloadLink.linkURL}"/>" title="${downloadLink.linkTitle}"><span class="icon  icon-functional" data-icon="="></span></a>  <span class="filesize"> (${downloadLink.fileSize})</span></td>
                     </tr>
                     </c:when>
                     <c:otherwise>
                    </c:otherwise>
             ...
             <c:forEach var="downloadLinkList" items="${model.downloadSection.sequencesDownloadSection.listOfChunkedDownloadLinks}" varStatus="loop">
                 <c:choose>
                                ...
                                <%--while implemented in file-definition-context--%>
                                    <c:choose>
                                      <c:when test="${downloadLinkList[0].linkPrefix == 'InterPro matches' || downloadLinkList[0].linkPrefix ==  'InterPro matches (TSV)'}">
                                          <span class="show_tooltip icon  icon-generic" data-icon="g" title="Chart data type"></span>
                                      </c:when>
                ...
                <c:if test="${(not empty model.downloadSection.functionalDownloadSection.interproscanDownloadLinks || not empty model.downloadSection.functionalDownloadSection.otherDownloadLinks)}">
                    <h4>Functional analysis</h4>
                       ...
                       <table class="pipeline_table">
                        ...
                        <c:forEach var="downloadLink" items="${model.downloadSection.functionalDownloadSection.otherDownloadLinks}" varStatus="loop">
                ...
                <c:if test="${not empty model.downloadSection.taxaAnalysisDownloadLinks}">
                    ...
                    <table class="pipeline_table">
                       ...
                       JSON Biom <span class="icon icon-generic show_tooltip" data-icon="i" title="BIOM is a standard format (http://biom-format.org/) for representing taxonomy distribution (OTUs and their occurence in each sample). BIOM files could be directly imported in diverse analysis packages such as MEGAN (http://ab.inf.uni-tuebingen.de/software/megan5/)."></span>
                       </c:when>
                        <c:when test="${downloadLink.linkText == 'Phylogenetic tree'}">
                        Newick format <span class="icon icon-generic show_tooltip" data-icon="i" title="The Newick format is a computer-readable phylogenetic tree representation (http://evolution.genetics.washington.edu/phylip/newicktree.html). The trees can be visualized using graphical viewers such as FigTree (http://tree.bio.ed.ac.uk/software/figtree/) and iTOL (http://itol.embl.de/)."></span>
                        </c:when>
                        <c:otherwise>
                            FASTA
                        </c:otherwise>
