# Console log
message(paste(Sys.time(),'[R - Message] Launched R script PCA_v2.R'))
...
PerformPCA = function(abTable,category) {
  ...
  varPercent = round(as.numeric(varPercent),4)
...
bioPCA = PerformPCA(abTable,'biological_process')
molPCA = PerformPCA(abTable,'molecular_function')
allBioPlots = c(DrawPCAPlot(bioPCA,1,2),DrawPCAPlot(bioPCA,1,3),DrawPCAPlot(bioPCA,2,1),DrawPCAPlot(bioPCA,2,3),DrawPCAPlot(bioPCA,3,1),DrawPCAPlot(bioPCA,3,2))
allMolPlots = c(DrawPCAPlot(molPCA,1,1),DrawPCAPlot(molPCA,1,2),DrawPCAPlot(molPCA,1,3),DrawPCAPlot(molPCA,2,1),DrawPCAPlot(molPCA,2,2),DrawPCAPlot(molPCA,2,3),DrawPCAPlot(molPCA,3,1),DrawPCAPlot(molPCA,3,2),DrawPCAPlot(molPCA,3,3))

return(c(allBioPlots,allMolPlots))
}
